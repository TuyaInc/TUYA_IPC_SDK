...
/*
用户需要实现的接口：
tuya_linux_wf_station_connect
__tuya_linux_get_snap_qrcode
其他接口理论上均可以直接使用，如果有用户有自己的实现可以替换掉
*/
static BOOL sniffer_set_done = FALSE;
OPERATE_RET tuya_linux_wf_station_connect(IN CONST CHAR *ssid,IN CONST CHAR *passwd)
{
    printf("tuya_linux_wf_station_connect: ssid:%s passwd:%s sniffer_set_done:%d\r\n", ssid, passwd,sniffer_set_done);
    if(sniffer_set_done)
    {
    ...
    printf("tuya_linux_wf_station_connect end\r\n");
    return OPRT_OK;
}

STATIC CHAR* __tuya_linux_get_snap_qrcode(VOID)
{
    return "1234";
}
...
OPERATE_RET tuya_linux_wf_all_ap_scan(OUT AP_IF_S **ap_ary,OUT UINT *num)
{
    ...
    printf("tuya_linux_wf_all_ap_scan\r\n");
    ...
    *num = 0;
    ...
    while (fgets(tmp, sizeof(tmp), pp) != NULL)
    {
        ...
        if(pBSSIDStart != NULL)
        {
            ...
            BYTE *pTmp = s_aps[recordIdx].bssid;
...
#endif
    printf("tuya_linux_wf_all_ap_scan end\r\n");
...
OPERATE_RET tuya_linux_wf_assign_ap_scan(IN CONST BYTE *ssid,OUT AP_IF_S **ap)
{
    ...
    printf("tuya_linux_wf_assign_ap_scan\r\n");
    ...
    */
    ...
    UINT tatalNum = 0;
    int index = 0;
    tuya_linux_wf_all_ap_scan(&pTotalAp, &tatalNum);
    ...
    *ap = NULL;
    ...
    printf("tuya_linux_wf_assign_ap_scan end\r\n");
    return OPRT_OK;
}

OPERATE_RET tuya_linux_wf_release_ap(IN AP_IF_S **ap)
{//静态变量，无需释放
    printf("tuya_linux_wf_release_ap\r\n");
    return OPRT_OK;
}
...
OPERATE_RET tuya_linux_wf_set_cur_channel(IN CONST BYTE chan)
{
    ...
    exec_cmd(tmpCmd);
    s_curr_channel = chan;
    printf("tuya_linux_wf_set_cur_channel:%d \r\n", chan);

#ifdef WIFI_CHIP_7601
    tuya_linux_wf_wk_mode_set(WWM_SNIFFER);
#endif

    printf("tuya_linux_wf_set_cur_channel end\r\n");
    return OPRT_OK;
}
...
OPERATE_RET tuya_linux_wf_get_cur_channel(OUT BYTE *chan)
{
    ...
    printf("tuya_linux_wf_get_cur_channel\r\n");

#ifdef __HuaweiLite__
    //todo liteos系统的信道获取实现
#else
    ...
    printf("tuya_linux_wf_get_cur_channel end chan: %d\r\n",*chan);
    return OPRT_OK;
}
...
{
    printf("tuya_linux_wf_sniffer_set %d\r\n",en);
    if(en == TRUE)
    {
    ...
    }else
    {
        ...
        tuya_linux_wf_wk_mode_set(WWM_STATION);
    ...
    printf("tuya_linux_wf_sniffer_set end\r\n");
    return OPRT_OK;
}
...
OPERATE_RET tuya_linux_wf_get_ip(IN CONST WF_IF_E wf,OUT NW_IP_S *ip)
{
    ...
    printf("tuya_linux_wf_get_ip wf %d\r\n",wf);
    ...
    printf("tuya_linux_wf_get_ip end: ip %s, mask %s, gw %s\r\n",ip->ip,ip->mask,ip->gw);
    return OPRT_OK;
}
...
。。。。。。
*/
OPERATE_RET tuya_linux_wf_get_mac(IN CONST WF_IF_E wf,INOUT NW_MAC_S *mac)
{
    if(NULL == mac)
    {
        return OPRT_INVALID_PARM;
    }
    printf("tuya_linux_wf_get_mac wf %d\r\n",wf);
    ...
    printf("tuya_linux_wf_get_mac %02X-%02X-%02X-%02X-%02X-%02X\r\n",
...
OPERATE_RET tuya_linux_wf_wk_mode_set(IN CONST WF_WK_MD_E mode)
{
    ...
    printf("tuya_linux_wf_wk_mode_set mode %d\r\n",mode);
    ...
    printf("tuya_linux_wf_wk_mode_set end %d\r\n",mode);
...
OPERATE_RET tuya_linux_wf_wk_mode_get(OUT WF_WK_MD_E *mode)
{
    ...
    printf("tuya_linux_wf_wk_mode_get\r\n");
    ...
    *mode = WWM_STATION;
    ...
    printf("tuya_linux_wf_wk_mode_get end: mode : %d\r\n",*mode);
...
{
    printf("tuya_linux_wf_station_disconnect\r\n");
    //Reserved
...
OPERATE_RET tuya_linux_wf_station_get_conn_ap_rssi(OUT SCHAR *rssi)
{
    ...
    *rssi = 0;
    printf("tuya_linux_wf_station_get_conn_ap_rssi\r\n");
    ...
    pclose(pp);
#endif
    printf("tuya_linux_wf_station_get_conn_ap_rssi end: rssi:%d\r\n",rssi);
...
OPERATE_RET tuya_linux_wf_station_stat_get(OUT WF_STATION_STAT_E *stat)
{
    ...
    printf("tuya_linux_wf_station_stat_get\r\n");
    *stat = WSS_GOT_IP;
    //Reserved
    return OPRT_OK;
}
...
OPERATE_RET tuya_linux_wf_ap_start(IN CONST WF_AP_CFG_IF_S *cfg)
{
    ...
    printf("tuya_linux_wf_ap_start:%s \r\n", cfg->ssid);
    //Reserved
    return OPRT_OK;
}
