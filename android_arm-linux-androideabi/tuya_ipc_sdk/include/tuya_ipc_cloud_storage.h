...
typedef INT (*Tuya_CBC_AES128_Init)(VOID);
typedef INT (*Tuya_CBC_AES128_Encrypt)(IN BYTE *pdata_in,   //加密前数据，需要保证不会被修改
                                             IN UINT data_len,     //加密前数据大小
                                             IN BYTE *pdata_out,   //机密后数据目的地址，SDK内部申请了空间，不需要额外malloc
                                             OUT UINT *pdata_out_len,   //加密后数据大小
                                             IN BYTE *pkey,     //aes加密key
                                             IN BYTE *piv);     //aes cbc加密iv
typedef INT (*Tuya_CBC_AES128_Destroy)(VOID);
 ...
 * \return OPERATE_RET
 */
OPERATE_RET tuya_ipc_cloud_storage_event_start(IN CHAR *snapshot_buffer, IN UINT snapshot_size, IN ClOUD_STORAGE_EVENT_TYPE_E type);
 ...
 * \return OPERATE_RET
 */
EVENT_STATUS_E tuya_ipc_cloud_storage_get_event_status(VOID);
