...
void *thread_stream_storage(void *arg)
{
    ...
    BOOL_T is_retry[2] = {FALSE,FALSE};
    OPERATE_RET ret;
    Ring_Buffer_Node_S *node_v;
    Ring_Buffer_Node_S *node_a;
    STORAGE_STATUS_E status;

    PR_DEBUG("stream_storage thread start");
    while(TRUE)
    {
        status = tuya_ipc_ss_get_status();
        if(E_STORAGE_STOP == status || E_STORAGE_INVALID == status)
        {
            usleep(50*1000);
            continue;
        }
        node_v = tuya_ipc_ring_buffer_get_video_frame(E_CHANNEL_VIDEO_MAIN,E_USER_STREAM_STORAGE,is_retry[0]);
        if(node_v != NULL)
        {
            frame.p_buf = node_v->rawData;
            frame.size = node_v->size;
            frame.type = node_v->type;
            frame.pts = node_v->pts;
            frame.timestamp = node_v->timestamp;
            ret = tuya_ipc_ss_put_frame(&frame);
            if(ret != OPRT_OK)
            {
                is_retry[0] = TRUE;
            }
        ...
        if(s_media_info.channel_enable[E_CHANNEL_AUDIO])
        {
            node_a = tuya_ipc_ring_buffer_get_audio_frame(E_CHANNEL_AUDIO,E_USER_STREAM_STORAGE,is_retry[1]);
            if(node_a != NULL)
            {
                frame.p_buf = node_a->rawData;
                frame.size = node_a->size;
                frame.type = node_a->type;
                frame.pts = node_a->pts;
                frame.timestamp = node_a->timestamp;
                ret = tuya_ipc_ss_put_frame(&frame);
                if(ret != OPRT_OK)
                {
                    is_retry[1] = TRUE;
        ...
        if(node_v == NULL && node_a == NULL)
        {
            SystemSleep(10);
        }
    }

    PR_DEBUG("stream_storage thread finish");

    pthread_exit(0);
}

#define DISK_CLEAN_THRESH           50    //触发删除文件的磁盘剩余空间阈值 MB
...
void *thread_disk_clean(void *arg)
{
    ...
    while (TRUE)
    {
        ...
        freeBytes = ((UINT64_T)space_stat.f_bavail * (UINT64_T)space_stat.f_bsize) >> 20;
        PR_DEBUG("curr free %s: %u", base_path, freeBytes);
        if(freeBytes < DISK_CLEAN_THRESH)
        {
            PR_DEBUG("curr free %s: %u < %u Delete.",
...
OPERATE_RET TUYA_APP_Init_Stream_Storage(IN CONST CHAR_T *p_sd_base_path)
{
    ...
    OPERATE_RET ret = tuya_ipc_ss_init(p_sd_base_path, &s_media_info);
    if(ret != OPRT_OK)
    {
    ...
    pthread_create(&stream_storage_thread, NULL, thread_stream_storage, NULL);
    pthread_detach(stream_storage_thread);

    pthread_t disk_clean_thread;
    STATIC CHAR_T base_dir[128];
    ...
    //tuya_ipc_ss_trigger_event(20);  //触发20秒的事件存储
...
OPERATE_RET TUYA_APP_Init_Ring_Buffer(VOID)
{
    ...
    for( channel = E_CHANNEL_VIDEO_MAIN; channel < E_CHANNEL_MAX; channel++ )
    {
        ...
        if(s_media_info.channel_enable[channel] == TRUE)
        {
            ...
            {
                PR_DEBUG("audio_sample %d, audio_databits %d, audio_fps %d",s_media_info.audio_sample,s_media_info.audio_databits,s_media_info.audio_fps);
                ret = tuya_ipc_ring_buffer_init(channel, s_media_info.audio_sample*s_media_info.audio_databits/1024,s_media_info.audio_fps,NULL);
            }
            else
            {
...
{  
    p_out->audio_codec = s_media_info.audio_codec;
    p_out->audio_sample = s_media_info.audio_sample;
    p_out->audio_databits = s_media_info.audio_databits;
    p_out->audio_channel = s_media_info.audio_channel;
...
STATIC VOID __TUYA_APP_rev_audio_cb(IN CONST TRANSFER_AUDIO_FRAME_S *p_audio_frame, IN CONST UINT_T time_stamp, IN CONST UINT_T frame_no)
{
...
OPERATE_RET TUYA_APP_Enable_P2PTransfer(IN UINT_T max_users)
{
    ...
    s_p2p_mgr.p2p_audio_codec = s_media_info.audio_codec;

    TUYA_IPC_TRANSFER_VAR_S p2p_var = {0};
    p2p_var.online_cb = __depereated_online_cb;
    p2p_var.on_rev_audio_cb = __TUYA_APP_rev_audio_cb;
    p2p_var.rev_audio_codec = s_media_info.audio_codec;
    p2p_var.audio_sample = s_media_info.audio_sample;
    p2p_var.audio_databits = s_media_info.audio_databits;
    p2p_var.audio_channel = s_media_info.audio_channel;
