...
VOID IPC_APP_Register_Media_Info_CB(INOUT IPC_MEDIA_INFO_S *p_media_info)
{
    ...
    /* 注意2： E_CHANNEL_VIDEO_MAIN必须存在。对于只有编码器只出一路流的情况，请关闭E_CHANNEL_VIDEO_SUB */
    ...
    p_media_info->video_bitrate[E_CHANNEL_VIDEO_MAIN] = TUYA_VIDEO_BITRATE_2M; /* 码率上限 */
    ...
    p_media_info->audio_codec = TUYA_CODEC_AUDIO_PCM;/* 编码格式 */
    p_media_info->audio_sample = TUYA_AUDIO_SAMPLE_8K;/* 采样率 */
    p_media_info->audio_databits = TUYA_AUDIO_DATABITS_16;/* 位宽 */
    p_media_info->audio_channel = TUYA_AUDIO_CHANNEL_MONO;/* 信道 */
    p_media_info->audio_fps = 25;/* 每秒分片数 */
}
