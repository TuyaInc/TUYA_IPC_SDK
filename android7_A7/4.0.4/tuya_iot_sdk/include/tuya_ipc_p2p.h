...
typedef struct
{
    ...
    BYTE_T *p_audio_buf; /**< 音频帧buffer */
    UINT_T buf_len;  /**< 音频帧buffer长度 */
    UINT64_T pts;     //音频采集显示时间戳
...
typedef struct
{
    ...
    BYTE_T *p_video_buf; /**< 视频帧buffer */
    UINT_T buf_len;  /**< 视频帧buffer长度 */
    UINT64_T pts; //音视频采集显示时间戳
    UINT64_T timestamp;   // Timestamp of the frame, in milliseconds
    VOID *p_reserved;
...
typedef struct
{
    ...
    INT_T max_client_num;/**< 最大支持客户端数量 */
...
/**
 * \fn OPERATE_RET tuya_ipc_get_p2p_conn_num(OUT INT_T * pNum)
 * \brief 获取当前p2p连接数目
 * \param[out] pNum 连接数目
 * \return OPERATE_RET
 */
 
 OPERATE_RET tuya_ipc_get_p2p_conn_num(OUT INT_T * pNum);
/**
 * \fn OPERATE_RET tuya_ipc_malloc_time_s(IN CONST UINT_T num, OUT PLAYBACK_TIME_S **p_p_time_arr)
 * \brief 获取查询的指针数组内存
 * \param[in] num 要获取的数量
 * \param[out] p_p_time_arr 内存指针
 * \return OPERATE_RET
*/
OPERATE_RET tuya_ipc_malloc_time_s(IN CONST UINT_T num, OUT PLAY_BACK_ALARM_INFO_ARR **p_p_time_arr);
