...
STATIC VOID __IPC_APP_Get_Wired_Status_cb(IN CONST GW_BASE_NW_STAT_T stat)
{
    PR_DEBUG("wired status changed:%d", stat);
    switch(stat)
    {
...
OPERATE_RET IPC_APP_Init_SDK(VOID)
{
    ...
    GW_PROD_INFO_S prod_info = {s_mgr_info.uuid, s_mgr_info.auth_key};
    ...
    tuya_iot_soc_init(&iot_cb, s_mgr_info.product_key, s_mgr_info.dev_sw_version);
    tuya_iot_reg_get_nw_stat_cb(__IPC_APP_Get_Wired_Status_cb);
